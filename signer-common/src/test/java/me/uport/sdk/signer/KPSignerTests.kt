...
class KPSignerTests {
    ...
    fun `sign ETH calls back without error`() {
        ...
        val rawTransactionBytes =
            "f380850ba43b7400832fefd8949e2068cce22de4e1e80f15cb71ef435a20a3b37c880de0b6b3a7640000890abcdef01234567890".hexToByteArray()
        ...
        signer.signETH(rawTransactionBytes) { err, sigData ->
            ...
            assertThat(sigData).isEqualTo(expectedSignature)
        }
    }
    ...
    fun `signJWT calls back with correct signature`() {
        ...
        signer.signJWT(referencePayload) { err, sigData ->
            assertThat(err).isNull()
            assertThat(sigData).isEqualTo(referenceSignature)
        }
    }
    ...
    fun `returned address is non null`() {
        val signer = KPSigner("65fc670d9351cb87d1f56702fb56a7832ae2aab3427be944ab8c9f2a0ab87960")
        assertThat(signer.getAddress()).isEqualTo("0x794adde0672914159c1b77dd06d047904fe96ac8")
    }
}